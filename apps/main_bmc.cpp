...
//要验证所有安全属性在 bound 步内同时成立。
//如果有一条不成立，整个 conjunct 就为 false，即发现反例
...
//sweeper
    ...
    int nodes_before = 0;
    count_total_nodes(root, nodes_before);
    ...
    int nodes_after = 0;
    count_total_nodes(root, nodes_after);

    std::cout << "[Sweeper Reduction] "
          << nodes_before << " -> " << nodes_after
          << " (" << (100.0 * (nodes_before - nodes_after) / nodes_before)
          << "%)\n";
    ...
    solver->push();
    //临时测试NOT(property)，后推出，不污染solver
