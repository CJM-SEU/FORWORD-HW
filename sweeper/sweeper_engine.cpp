...
void post_order(const Term & root,// 表达式根节点
                std::unordered_map<Term, NodeData> & node_data_map,// 节点 → 仿真数据
                std::unordered_map<uint32_t, TermVec> & hash_term_map,// 仿真哈希 → 等价候选节点列表
                std::unordered_map<Term, Term> & substitution_map,// 原节点 → 替换节点（核心输出）
    ...
    const int NEED_SAT   = std::max(0, required_sat);
     ...
     //——— 相位管理 ———//
    bool apply_only = false;        // // false: 相位A（发现）"找等价 + 验证"  ；true: 相位B（仅应用）
//用显式栈模拟递归：先处理children，再处理root
    while(!node_stack.empty()) {
        ...
        if(current->is_value()) {
            if(!apply_only){
                ...
                // 已收集足够样本，跳过仿真和哈希索引，仅做映射
                substitution_map.insert_or_assign(current, current);
            }
            processed_nodes++;// 计数器 +1（用于进度百分比）
            node_stack.pop();// 从栈中弹出，继续处理下一个
...
//调用仿真器
    simulation(free_symbols,
               ...
               load_path,
               constraints);
//校验每个自由符号仿真数据完整，然后将其纳入/注册全局哈希索引，
//确保 post_order 在查找候选等价项时能看到整棵表达式的所有叶子
